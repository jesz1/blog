<x-jet-action-section>
    <x-slot name="title">
        {{ __('Factor de dos pasos') }}
    </x-slot>

    <x-slot name="description">
        {{ __('agrega seguridad adicional a tu cuenta') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('activaste la authenticacion') }}
                @else
                    {{ __('desactivaste la authenticacion') }}
                @endif
            @else
                {{ __('Realiza la Autheticacion') }}
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Inicios mas rapidos ') }}
            </p>
        </div>
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        ...
                        @else
                            {{ __('SCANNEA EL CODIGO EN TU TELEFONO O UTILIZA ESTAS DE REFRERENCIA') }}
                        @endif
                    </p>
                </div>
